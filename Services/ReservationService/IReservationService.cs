...
namespace f00die_finder_be.Services.ReservationService
{
    ...
    {
        Task<CustomResponse<List<ReservationDto>>> GetReservationsOfMyRestaurantAsync(FilterReservationDto filterReservationOfRestaurantDto, int pageSize, int pageNumber);
        Task<CustomResponse<List<ReservationDto>>> GetMyReservations(FilterReservationDto filterReservationDto, int pageSize, int pageNumber); 
